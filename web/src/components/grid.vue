      ...
      <tr v-for="(item, index) in listData">
        ...
        <td  v-for="(item2, index2) in theadData">
          <span v-if="index2 === 0" style="float: right;">
              <i title="编辑" v-if="ifEdit" class="icon-edit" @click="editHandler(item)"></i>
              <i title="删除" v-if="ifDelete" class="icon-trash" @click="deleteHandler(item)"></i>
              <i title="下移" v-if="ifDown" class="icon-chevron-down" @click="downHandler(item)"></i>
              <i title="上移" v-if="ifUp" class="icon-chevron-up" @click="upHandler(item)"></i>
              <i title="封号" v-if="ifReset" class="icon-unlock-alt" @click="resetHandler(item)"></i>
          </span>
          {{item[item2.keyname]}}
        </td>
      </tr>
